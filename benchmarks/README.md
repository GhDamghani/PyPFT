# Benchmarks

This directory is reserved for PyPFT performance benchmarks.

The benchmark suite will be added after the first DHT implementation exists. At that point the benchmark matrix should compare at least:

- single-image forward and backward execution
- batched forward and backward execution
- mock-mirror versus future reference versus optimized DHT selections
- representative small, medium, and larger grid sizes
