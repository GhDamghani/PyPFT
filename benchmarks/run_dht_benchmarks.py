"""Run bench_dht.py under pytest-benchmark and export results sorted by speed.

Usage: ``uv run python benchmarks/run_dht_benchmarks.py``

Writes a Markdown report to ``.local_files/benchmarks/results/`` (gitignored, since
reports are generated local artifacts, not source) and prints the
fastest ``DHTImplementation`` for the "forward, repeated calls" scenario -- the
one that should be hardcoded as ``pypft.dht.DEFAULT_IMPLEMENTATION``.
"""
...
def main() -> None:
    ...
    json_path = RESULTS_DIR / "_raw_benchmark.json"
    ...
    report_lines = [f"# DHT benchmark results ({timestamp})", ""]
