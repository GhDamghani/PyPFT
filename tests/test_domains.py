...
def test_each_subclass_is_tagged_with_its_own_domain():
    ...
    grid = PolarGrid(_N_RADIAL, _N_ANGULAR, _R)
    values = np.zeros((_N_RADIAL, _N_ANGULAR), dtype=complex)
    assert SpacePolarSignal(values, grid).domain is Domain.SPACE_POLAR
    assert SpaceHarmonicSignal(values, grid).domain is Domain.SPACE_HARMONIC
    assert FrequencyHarmonicSignal(values, grid).domain is Domain.FREQUENCY_HARMONIC
    assert FrequencyPolarSignal(values, grid).domain is Domain.FREQUENCY_POLAR
...
def test_construction_rejects_a_non_ndarray_values_argument():
    ...
    with pytest.raises(TypeError):
        SpacePolarSignal([[0.0]], grid)  # type: ignore[arg-type]
...
def test_the_polar_harmonic_edge_round_trips():
    ...
    original = SpacePolarSignal(_random_values(rng), grid)
...
def test_the_space_frequency_edge_round_trips():
    ...
    original = SpaceHarmonicSignal(_random_values(rng), grid)
...
def test_the_harmonic_polar_edge_round_trips():
    ...
    original = FrequencyHarmonicSignal(_random_values(rng), grid)
...
def test_to_composes_the_full_forward_chain_matching_forward_pft():
    ...
    grid = PolarGrid(_N_RADIAL, _N_ANGULAR, _R)
    f = np.exp(-(grid.r.T**2))
    signal = SpacePolarSignal(f, grid)
...
def test_to_composes_the_full_backward_chain_matching_inverse_pft():
    ...
    signal = FrequencyPolarSignal(F, grid)
...
def test_to_a_signals_own_domain_is_a_no_op():
    ...
    signal = SpaceHarmonicSignal(_random_values(rng), grid)
...
def test_to_rejects_a_non_domain_type():
    ...
    grid = PolarGrid(_N_RADIAL, _N_ANGULAR, _R)
    signal = SpacePolarSignal(np.zeros((_N_RADIAL, _N_ANGULAR), dtype=complex), grid)
    with pytest.raises(TypeError):
...
def test_a_hand_written_illegal_edge_is_not_a_valid_attribute():
    ...
    signal: BaseSignal = SpacePolarSignal(
        np.zeros((_N_RADIAL, _N_ANGULAR), dtype=complex), grid
    )
