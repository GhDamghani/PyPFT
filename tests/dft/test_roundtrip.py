...
def test_inverse_undoes_forward_at_both_parities(n_angular, implementation):
    ...
    roundtripped = inverse_angular_dft(angular_dft(x, implementation), implementation)
...
def test_implementations_agree_on_a_random_signal(n_angular):
    ...
    forward_results = [
        angular_dft(x, implementation) for implementation in DFTImplementation
    ]
    inverse_results = [
        inverse_angular_dft(x, implementation) for implementation in DFTImplementation
    ]
...
def test_angular_dft_axis_matches_looping_over_other_axes(implementation):
    ...
    batched = angular_dft(batch, implementation, axis=0)

    for i in range(batch.shape[1]):
        expected = angular_dft(batch[:, i], implementation)
